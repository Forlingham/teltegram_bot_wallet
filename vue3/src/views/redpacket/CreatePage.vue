...
const expireOptions = computed(() => {
  // Recompute when locale changes so labels follow the language selection.
  void locale.value
  const opts = [
    { value: 86400, label: t('common.unit.hour24') },
    { value: 604800, label: t('common.unit.days7') },
    { value: 31536000, label: t('common.unit.forever') },
  ]
  if (networkStore.appEnv !== 'production') {
    opts.unshift({ value: 180, label: t('common.unit.minutes3') })
  }
  return opts
})
...
async function updateEstimate() {
  ...
  const blessMessage = message.value.trim() || t('redpacketCreate.defaultBless')
...
function validate(): { ok: boolean; message?: string; totalSat?: bigint; countInt?: number } {
  ...
  if (!total) return { ok: false, message: t('redpacketCreate.errorEmptyAmount') }
  if (!totalSat || totalSat <= 0n) return { ok: false, message: t('redpacketCreate.errorBadAmount') }
  if (parseFloat(total) > MAX_TOTAL_SCASH) return { ok: false, message: t('redpacketCreate.errorTooLarge') }
  if (!countInt || countInt <= 0) return { ok: false, message: t('redpacketCreate.errorCount') }
  if (countInt > MAX_PACKET_COUNT) return { ok: false, message: t('redpacketCreate.errorCountMax', { max: MAX_PACKET_COUNT }) }
  if (Array.from(message.value.trim()).length > 14) return { ok: false, message: t('redpacketCreate.errorMessageLength') }
  ...
  if (perPacketSat < 100000n) return { ok: false, message: t('redpacketCreate.errorMinPerPacket') }
  ...
  if (availableBalance.value > 0n && estimatedTotal > availableBalance.value) {
    return { ok: false, message: t('redpacketCreate.errorInsufficient') }
  }
...
async function handlePasswordConfirm(password: string) {
  ...
  try {
    ...
    if (!balanceData) throw new Error(t('redpacketCreate.errorBalance'))

    const candidateUtxos = balanceData.utxos
      .filter((u) => !u.isUnconfirmed)
      .concat(balanceData.utxos.filter((u) => u.isUnconfirmed))
    if (candidateUtxos.length === 0) throw new Error(t('redpacketCreate.errorNoUtxo'))
    ...
    if (!poolAddr) throw new Error(t('redpacketCreate.errorPool'))
...
function shareToChat() {
  const shareText = t('redpacketCreate.shareText')
...
<template>
  <div>
    ...
    <div v-if="!showSuccess" class="space-y-4 px-4 pb-6">
      ...
      <section class="relative overflow-hidden p-4 rounded-lg bg-surface-container-low">
        ...
        <div class="relative z-10 flex justify-between items-center">
          <div>
            <span class="text-[10px] font-semibold text-on-surface-variant uppercase tracking-wider mb-0.5 block">{{ t('redpacketCreate.availableBalance') }}</span>
      ...
      <section class="grid grid-cols-2 gap-3">
        ...
        >
          ...
          <span class="text-xs font-bold" :class="redpacketType === 'RANDOM' ? 'text-on-surface' : 'text-on-surface-variant'">{{ t('redpacketCreate.typeRandom') }}</span>
        </button>
        <button
        ...
        >
          ...
          <span class="text-xs font-bold" :class="redpacketType === 'EQUAL' ? 'text-on-surface' : 'text-on-surface-variant'">{{ t('redpacketCreate.typeEqual') }}</span>
        </button>
      </section>
      ...
      <section class="space-y-3">
        <div class="p-4 rounded-lg bg-surface-container-lowest shadow-[0px_4px_16px_rgba(44,47,49,0.02)]">
          <div class="flex justify-between items-center mb-2">
            <label class="text-xs font-bold text-on-surface-variant">{{ t('redpacketCreate.totalAmount') }}</label>
        ...
        <div class="p-4 rounded-lg bg-surface-container-lowest shadow-[0px_4px_16px_rgba(44,47,49,0.02)]">
          <div class="flex justify-between items-center mb-2">
            <label class="text-xs font-bold text-on-surface-variant">{{ t('redpacketCreate.packetCount') }}</label>
            <span class="text-[10px] font-semibold text-on-surface-variant">{{ t('redpacketCreate.packetUnit') }}</span>
          </div>
      ...
      <!-- Settings -->
      <section class="space-y-3">
        <div class="text-[9px] font-extrabold uppercase tracking-widest text-on-surface-variant/70 px-1">{{ t('redpacketCreate.settings') }}</div>
        <div class="rounded-lg bg-surface-container-low overflow-hidden">
          <div class="flex items-center justify-between p-4 bg-surface-container-lowest mb-[1px]">
            <span class="text-xs font-semibold text-on-surface">{{ t('redpacketCreate.messageLabel') }}</span>
            <div class="flex items-center gap-2 flex-1 ml-4">
              <input v-model="message" class="w-full text-right bg-transparent border-none focus:ring-0 p-0 text-xs text-on-surface placeholder:text-on-surface-variant/40" :placeholder="t('redpacketCreate.messagePlaceholder')" type="text" maxlength="14" @input="debouncedEstimate" />
            </div>
          </div>
          <div class="p-4 bg-surface-container-lowest mb-[1px]">
            <div class="flex items-center justify-between mb-3">
              <span class="text-xs font-semibold text-on-surface">{{ t('redpacketCreate.expiryLabel') }}</span>
              <span class="text-[10px] text-primary font-bold">{{ expireOptions.find(o => o.value === expireSeconds)?.label || t('common.unit.hour24') }}</span>
            </div>
          ...
          <div class="p-4 bg-surface-container-lowest">
            <span class="text-xs font-semibold text-on-surface block mb-3">{{ t('redpacketCreate.coverLabel') }}</span>
            <div class="text-on-surface-variant text-xs">{{ t('redpacketCreate.coverNone') }}</div>
          </div>
        </div>
      </section>
      ...
      <section v-if="estimateData" class="p-4 rounded-lg bg-surface-container-low/50 border border-outline-variant/20">
        <div class="mb-3">
          <h3 class="text-xs font-bold text-on-surface-variant">{{ t('redpacketCreate.feeTitle') }}</h3>
        </div>
        <div class="space-y-2">
          <div class="flex justify-between items-center">
            <span class="text-[10px] text-on-surface-variant">{{ t('redpacketCreate.feeAmount') }}</span>
            <span class="text-[10px] font-medium text-on-surface">{{ estimateData.amountSat ? satsToScash(estimateData.amountSat) : '-' }} SCASH</span>
          </div>
          <div class="flex justify-between items-center">
            <span class="text-[10px] text-on-surface-variant">{{ t('redpacketCreate.feeReserve') }}</span>
            <span class="text-[10px] font-medium text-on-surface">{{ satsToScash(estimateData.reserveSat) }} SCASH</span>
          </div>
          <div class="flex justify-between items-center">
            <span class="text-[10px] text-on-surface-variant">{{ t('redpacketCreate.feeNetwork') }}</span>
            <span class="text-[10px] font-medium text-on-surface">{{ satsToScashTrimmed(NETWORK_FEE_SAT) }} SCASH</span>
          </div>
          <div class="flex justify-between items-center">
            <span class="text-[10px] text-on-surface-variant">{{ t('redpacketCreate.feeApp') }}</span>
            <span class="text-[10px] font-medium text-on-surface">{{ satsToScashTrimmed(ARR_FEE_SAT) }} SCASH</span>
          </div>
          <div class="flex justify-between items-center">
            <span class="text-[10px] text-on-surface-variant">{{ t('redpacketCreate.feeDap') }}</span>
            <span class="text-[10px] font-medium text-on-surface">{{ estimateData.dapCostSat ? satsToScashTrimmed(estimateData.dapCostSat) + ' SCASH' : t('redpacketCreate.feeDapCalculating') }}</span>
          </div>
          <div class="pt-2 mt-1 border-t border-outline-variant/10 flex justify-between items-baseline">
            <span class="text-xs font-bold text-on-surface">{{ t('redpacketCreate.feeTotal') }}</span>
      ...
      >
        ...
        <template v-else>
          ...
          <span class="font-headline font-bold text-white text-base tracking-wide">{{ t('redpacketCreate.submit') }}</span>
        </template>
      </button>
    </div>
    ...
    <div v-if="showSuccess" class="min-h-screen flex flex-col items-center px-4 py-6 relative overflow-hidden">
      ...
      <div class="flex items-center justify-between mb-6 relative w-full max-w-md px-2">
        <div class="flex items-center gap-3">
          ...
          <div class="text-left">
            <h1 class="font-headline font-extrabold text-xl tracking-tight text-on-surface">{{ t('redpacketCreate.successTitle') }}</h1>
            <p class="text-xs text-on-surface-variant">{{ t('redpacketCreate.successSubtitle', { amount: successTotal, count }) }}</p>
          </div>
        </div>
      ...
      <div class="w-full max-w-md space-y-4">
        ...
        <button class="w-full h-12 signature-gradient text-white font-headline font-bold rounded-full flex items-center justify-center gap-2 shadow-lg shadow-primary/20 active:scale-[0.98] transition-transform" @click="shareToChat">
          ...
          {{ t('redpacketCreate.shareToChat') }}
        </button>

        <!-- Transaction ID -->
        <div class="bg-surface-container-lowest p-4 rounded-lg shadow-[0px_12px_32px_rgba(44,47,49,0.04)]">
          <div class="flex items-center justify-between mb-2">
            <span class="text-[10px] text-on-surface-variant uppercase tracking-widest">{{ t('redpacketCreate.txIdLabel') }}</span>
            <a
              v-if="successTxid"
        ...
        <div class="bg-surface-container-lowest p-4 rounded-lg shadow-[0px_12px_32px_rgba(44,47,49,0.04)]">
          <div class="flex items-center gap-2 mb-2">
            ...
            <div class="flex-1 min-w-0">
              <h3 class="font-headline font-bold text-on-surface text-sm">{{ t('redpacketCreate.linkTitle') }}</h3>
              <p class="text-[10px] text-on-surface-variant">{{ t('redpacketCreate.linkHint') }}</p>
            </div>
            <button
        ...
        <router-link to="/wallet/redpacket" class="w-full h-12 bg-transparent border-2 border-outline-variant/20 text-on-surface-variant font-headline font-bold rounded-full flex items-center justify-center hover:bg-surface-container-low active:scale-[0.98] transition-all">
          {{ t('redpacketCreate.returnToCenter') }}
        </router-link>
      </div>
    </div>
        ...
        <div v-if="showConfirm" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div class="relative bg-surface-container-lowest rounded-lg shadow-xl w-full max-w-md max-h-[90vh] flex flex-col overflow-hidden">
            <div class="p-4 text-center shrink-0">
              ...
              <h1 class="text-lg font-headline font-bold text-on-surface">{{ t('redpacketCreate.confirmTitle') }}</h1>
            </div>
            <div class="px-6 py-2 space-y-3 overflow-y-auto">
              <div class="bg-surface-container-low rounded-lg p-4 flex flex-col items-center justify-center">
                <div class="text-[10px] text-on-surface-variant uppercase tracking-widest mb-1">{{ t('redpacketCreate.confirmAmount') }}</div>
                <div class="flex items-baseline gap-1">
              ...
              <div class="space-y-2 bg-surface p-3 rounded-lg text-xs">
                <div class="flex justify-between items-center">
                  <span class="text-on-surface-variant">{{ t('redpacketCreate.confirmType') }}</span>
                  <span class="text-on-surface font-semibold">{{ redpacketType === 'RANDOM' ? t('redpacketCreate.typeRandom') : t('redpacketCreate.typeEqual') }}</span>
                </div>
                <div class="flex justify-between items-center">
                  <span class="text-on-surface-variant">{{ t('redpacketCreate.confirmCount') }}</span>
                  <span class="text-on-surface font-semibold">{{ count }}{{ t('redpacketCreate.packetUnit') ? ' ' + t('redpacketCreate.packetUnit') : '' }}</span>
                </div>
                <div class="flex justify-between items-center">
                  <span class="text-on-surface-variant">{{ t('redpacketCreate.confirmMessage') }}</span>
                  <span class="text-on-surface font-semibold">{{ message || t('redpacketCreate.defaultBless') }}</span>
                </div>
                <div class="flex justify-between items-center">
                  <span class="text-on-surface-variant">{{ t('redpacketCreate.confirmExpiry') }}</span>
                  <span class="text-on-surface font-semibold">{{ expireOptions.find(o => o.value === expireSeconds)?.label || t('common.unit.hour24') }}</span>
                </div>
              </div>
              <div class="flex justify-between items-center px-2 pb-2">
                <span class="text-[10px] text-on-surface-variant">{{ t('redpacketCreate.confirmTotal') }}</span>
            ...
            <div class="p-4 shrink-0 flex flex-col gap-2">
              <button class="w-full h-12 bg-gradient-to-r from-primary to-primary-container text-white rounded-full font-headline font-bold shadow-lg shadow-primary/20 active:scale-[0.98] transition-transform flex items-center justify-center gap-2" @click="handleConfirmOk">
                ...
                {{ t('redpacketCreate.confirmSend') }}
              </button>
              <button class="w-full h-12 bg-surface-container-high text-on-surface-variant rounded-full font-headline font-semibold active:scale-[0.98] transition-transform" @click="showConfirm = false">{{ t('common.cancel') }}</button>
            </div>
          </div>
        </div>
      </Transition>
    ...
    <PasswordModal
      ...
      :title="t('redpacketCreate.passwordTitle')"
      :confirm-text="t('redpacketCreate.passwordConfirm')"
      :loading="submitting"
