...
<template>
  <div>
    ...
    <!-- Success screen -->
    <div v-if="showSuccess" class="min-h-screen flex flex-col items-center justify-center px-4 py-6 relative overflow-hidden">
      ...
      <div class="w-full max-w-md space-y-4">
        <div class="bg-surface-container-low rounded-lg text-center">
          <span class="text-[10px] text-on-surface-variant uppercase tracking-widest block mb-1">红包总额</span>
          <div class="flex items-center justify-center gap-1">
            <span class="font-headline font-bold text-xl text-primary">{{ successTotal }}</span>
          ...
          <div class="border-t border-outline-variant/10">
            <div class="flex items-center justify-center gap-2 bg-surface-container-lowest py-1.5 px-3 rounded-full">
              <span class="text-[10px] text-on-surface-variant truncate max-w-[200px]">{{ successPacketHash }}</span>
              <button class="text-primary hover:scale-110 active:scale-95 transition-transform" @click="copyToClipboard(successPacketHash)">
        ...
        <div class="bg-surface-container-lowest p-4 rounded-lg shadow-[0px_12px_32px_rgba(44,47,49,0.04)] relative overflow-hidden">
          <div class="flex items-start gap-3">
            ...
            <div class="flex-1 min-w-0">
              ...
              <p class="text-[10px] text-on-surface-variant leading-tight mb-2">分享此链接给好友，即可让他们领取您的红包。</p>
              <div class="bg-surface p-2 rounded-lg flex items-center justify-between cursor-pointer hover:bg-surface-container-high transition-colors" @click="copyToClipboard(successShareUrl)">
                <span class="text-[10px] text-primary font-medium truncate flex-1 pr-2">{{ successShareUrl }}</span>
