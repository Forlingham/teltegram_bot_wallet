...
async function handleScanQr() {
  try {
    const raw = await showScanQr(t('send.scanPrompt'))
    let addr = raw.trim()
    ...
    if (!addr) {
      await showAlert(t('send.errorScanNoAddress'))
      return
    }
    ...
    if (!addr.startsWith(prefix + '1')) {
      await showAlert(t('send.errorScanWrongPrefix', { prefix }))
      return
    }
...
function validate(): { ok: boolean; message?: string } {
  ...
  if (!addr) return { ok: false, message: t('send.errorEmptyAddress') }
  if (!validateAddressFormat(addr)) {
    const prefix = networkStore.bech32 || 'scash'
    return { ok: false, message: t('send.errorBadPrefix', { prefix }) }
  }
  const amt = amount.value.trim()
  if (!amt) return { ok: false, message: t('send.errorEmptyAmount') }
  const amountSat = parseScashToSats(amt)
  if (!amountSat || amountSat <= 0n) return { ok: false, message: t('send.errorBadAmount') }
  if (parseFloat(amt) <= 0) return { ok: false, message: t('send.errorZeroAmount') }
  ...
  if (availableBalance.value > 0n && totalNeed > availableBalance.value) {
    return { ok: false, message: t('send.errorInsufficient') }
  }
  return { ok: true }
}
...
async function handlePasswordConfirm(password: string) {
  ...
  try {
    ...
    await showAlert(t('send.successAlert', { txid }))
    router.push('/wallet/history')
  } catch (e: any) {
    const msg = e?.message || e?.status?.message || t('send.errorSendFailed')
    errorMsg.value = msg
  ...
  <div class="space-y-6 px-4 pb-6">
    ...
    <section class="space-y-3">
      <div class="flex justify-between items-end px-2">
        <label class="font-headline font-bold text-sm text-on-surface-variant">{{ t('send.addressLabel') }}</label>
      </div>
    ...
    <section class="space-y-3">
      <div class="flex justify-between items-end px-2">
        <label class="font-headline font-bold text-sm text-on-surface-variant">{{ t('send.amountLabel') }}</label>
        <span class="text-xs font-semibold text-tertiary">{{ t('send.amountAvailable', { amount: availableBalanceScash }) }}</span>
      </div>
      <div class="relative">
        <input
          v-model="amount"
    ...
    <!-- Message / DAP -->
    <section class="space-y-3">
      <label class="font-headline font-bold text-sm text-on-surface-variant px-2">{{ t('send.messageLabel') }}</label>
      <textarea
        v-model="message"
    ...
    <section v-if="estimateData" class="bg-surface-container-low rounded-lg p-6 space-y-4">
      <div class="flex items-center gap-2 mb-2">
        ...
        <h3 class="font-headline font-bold text-sm text-on-surface">{{ t('send.feeTitle') }}</h3>
      </div>
      <div class="space-y-3">
        <div class="flex justify-between items-center">
          <span class="text-sm text-on-surface-variant font-medium">{{ t('send.feeAmount') }}</span>
        ...
        <div class="flex justify-between items-center">
          <span class="text-sm text-on-surface-variant font-medium">{{ t('send.feeNetwork') }}</span>
          <span class="text-sm text-tertiary font-bold font-headline">{{ satsToScashTrimmed(estimateData.networkFeeSat) }} SCASH</span>
        </div>
        <div class="flex justify-between items-center">
          <span class="text-sm text-on-surface-variant font-medium">{{ t('send.feeApp') }}</span>
          <span class="text-sm text-tertiary font-bold font-headline">{{ satsToScashTrimmed(estimateData.arrFeeSat) }} SCASH</span>
        </div>
        <div class="flex justify-between items-center">
          <span class="text-sm text-on-surface-variant font-medium">{{ t('send.feeDap') }}</span>
      ...
      <div class="pt-4 mt-2 border-t border-outline-variant/10 flex justify-between items-center">
        <span class="text-sm font-bold text-on-surface">{{ t('send.feeTotal') }}</span>
    ...
    <footer class="pt-2">
      ...
      >
        ...
        <span v-else class="text-white font-headline font-extrabold text-lg">{{ t('send.submit') }}</span>
      ...
      <div class="mt-6 flex flex-col items-center gap-4">
        <div class="flex items-center gap-2 px-4 py-2 bg-surface-container-high/50 rounded-full">
          ...
          <span class="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest">{{ t('send.networkSecure') }}</span>
        </div>
      </div>
    </footer>
    ...
    <PasswordModal
      ...
      :confirm-text="t('send.passwordConfirm')"
      :loading="submitting"
